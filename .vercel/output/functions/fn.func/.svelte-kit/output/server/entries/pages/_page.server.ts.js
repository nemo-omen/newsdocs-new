...
function getInmatesData(document) {
  const inmateDivs = document?.querySelectorAll(
    ".inmate_div"
  );
  const inmates = [];
...
function buildInmateObject(name, src, dataKeys, dataVals) {
  ...
  inmate.charges = inmate.charges.map(
    (charge) => charge.replace(/^[0-9]{8}\s/g, "")
  );
...
async function parse(current, released) {
  ...
  const dateString = `From 7 a.m. on ${dayBefore.toLocaleDateString("en-US", shortDateOpts)}, to 7 a.m. ${today}`;
